...
class AgentService implements LoggerAwareInterface
{
    ...
    private function buildChoiceUiHint(): string
    {
        return "\n\n# Asking the user to choose"
            ...
            . "\nWhen the choice is about images, add an image reference to each option and the"
            . " frontend renders the options as clickable thumbnails. Two kinds of reference:"
            . "\n- `uid` (integer): the `sys_file` UID of an image you ALREADY received in this"
            . " conversation (e.g. via ExtractImages or ReadFile, each shown to you inline with a"
            . " `sys_file:<uid>` marker)."
            . "\n- `url` (string): a direct http(s) image URL for images that are NOT sys_file records,"
            . " e.g. images found in tool results or on the web."
            . "\n\n```agent-choices"
            . "\n{\"question\": \"Welche Bilder soll ich einbauen?\", \"multiselect\": true, \"options\": ["
            . "{\"label\": \"Logo oben links\", \"uid\": 123}, "
            . "{\"label\": \"Stock-Foto\", \"url\": \"https://example.com/photo.jpg\"}]}"
            . "\n```"
            . "\n- Use `uid` ONLY with the real sys_file UID of an image you actually received in this"
            . " conversation (from its inline `sys_file:<uid>` marker) — never invent UIDs. Likewise,"
            . " use `url` only with a real image URL you actually encountered — never invent URLs."
            . "\n- Give each image a short, distinguishing `label`."
            . "\n- The click comes back as a message listing the chosen labels with their refs, e.g."
            . " `Ausgewählte Bilder: Logo oben links (sys_file:123), Stock-Foto (https://example.com/photo.jpg)`."
            . " To place a chosen sys_file image, reference its UID directly as `uid_local` in a"
            . " WriteTable sys_file_reference — scratch images are auto-promoted to public storage"
            . " on write, so you do NOT need to call PromoteScratchFile first.";
    }
